...
export POSTGRES_USER=$(vault read -format=json secret/data/app/database | jq -r '.data.data.user')
export DB_PASSWORD=$(vault read -format=json secret/data/app/database | jq -r '.data.data.password')
export POSTGRES_DB=$(vault read -format=json secret/data/app/database | jq -r '.data.data.database')
export JWT_SECRET=$(vault read -format=json secret/data/app/auth | jq -r '.data.data.jwt_secret')
export JWT_ISSUER=$(vault read -format=json secret/data/app/auth | jq -r '.data.data.jwt_issuer')
export JWT_AUDIENCE=$(vault read -format=json secret/data/app/auth | jq -r '.data.data.jwt_audience')
export SESSION_AUTH_KEY=$(vault read -format=json secret/data/app/auth | jq -r '.data.data.session_auth_key')
export SESSION_ENCRYPTION_KEY=$(vault read -format=json secret/data/app/auth | jq -r '.data.data.session_encryption_key')
export DOMAIN=$(vault read -format=json secret/data/app/config | jq -r '.data.data.domain')
export COOKIE_DOMAIN=$(vault read -format=json secret/data/app/config | jq -r '.data.data.cookie_domain')
export ALLOWED_ORIGINS=$(vault read -format=json secret/data/app/config | jq -r '.data.data.allowed_origins')
export APP_PORT=$(vault read -format=json secret/data/app/config | jq -r '.data.data.port')
export VITE_BACKEND_URL=$(vault read -format=json secret/data/app/frontend | jq -r '.data.data.backend_url')
